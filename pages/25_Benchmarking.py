...
with tab4:
    ...
    with mc:
        ...
        Bien. Justo después de la última línea añade esto:
python# ── DESCARGAS ─────────────────────────────────────────────────────────────
st.divider()
...
h2{{color:#1d4ed8;border-left:4px solid #1d4ed8;padding-left:12px;margin-top:28px;}}
...
.fortaleza{{background:#f0fdf4;border-left:4px solid #10b981;padding:12px 16px;margin:8px 0;border-radius:0 8px 8px 0;}}
.mejora{{background:#fef2f2;border-left:4px solid #ef4444;padding:12px 16px;margin:8px 0;border-radius:0 8px 8px 0;}}
.cuadrante{{background:#eff6ff;border-left:4px solid #1d4ed8;padding:12px 16px;margin:8px 0;border-radius:0 8px 8px 0;}}
...
<h1>Informe de Benchmarking Estratégico</h1>
<div class="perfil">
<strong>Empresa:</strong> {sector_dl} · {tam_dl} · {reg_dl} · <strong>Fecha:</strong> {hoy_dl}
</div>

<h2>1. Posición Competitiva Global</h2>
<table>
<tr><th>Índice</th><th>Valor</th><th>Interpretación</th></tr>
<tr><td>Score Estratégico Global (SSG)</td><td><strong>{ssg_dl}/100</strong></td><td>{'Posición sólida — tercio superior' if ssg_dl>70 else 'Posición intermedia — margen de mejora' if ssg_dl>50 else 'Posición débil — mejora urgente'}</td></tr>
<tr><td>ICE · Competitividad Empresarial</td><td>{ice_dl}/100</td><td>{'Alto' if ice_dl>70 else 'Medio' if ice_dl>40 else 'Bajo'}</td></tr>
<tr><td>ISF · Solidez Financiera</td><td>{isf_dl}/100</td><td>{'Alta' if isf_dl>70 else 'Media' if isf_dl>40 else 'Baja'}</td></tr>
<tr><td>IEO · Eficiencia Operativa</td><td>{ieo_dl}/100</td><td>{'Alta' if ieo_dl>70 else 'Media' if ieo_dl>40 else 'Baja'}</td></tr>
<tr><td>IDC · Dinamismo y Crecimiento</td><td>{idc_dl}/100</td><td>{'Alto' if idc_dl>70 else 'Medio' if idc_dl>40 else 'Bajo'}</td></tr>
<tr><td>IIE · Intensidad Exportadora</td><td>{iie_dl}/100</td><td>{'Alta' if iie_dl>70 else 'Media' if iie_dl>40 else 'Baja'}</td></tr>
<tr><td>IPT · Productividad y Talento</td><td>{ipt_dl}/100</td><td>{'Alto' if ipt_dl>70 else 'Medio' if ipt_dl>40 else 'Bajo'}</td></tr>
</table>

<h2>2. Perfil de Innovación</h2>
<table>
<tr><th>Indicador</th><th>Puntuación</th><th>Nivel</th></tr>
<tr><td>I+D+i</td><td>{b1_dl}/5</td><td>{'Alto' if b1_dl>3.5 else 'Medio' if b1_dl>2.5 else 'Bajo'}</td></tr>
<tr><td>Gestión de Proyectos</td><td>{b2_dl}/5</td><td>{'Alto' if b2_dl>3.5 else 'Medio' if b2_dl>2.5 else 'Bajo'}</td></tr>
<tr><td>Desarrollo de Productos</td><td>{b3_dl}/5</td><td>{'Alto' if b3_dl>3.5 else 'Medio' if b3_dl>2.5 else 'Bajo'}</td></tr>
<tr><td>Estrategia de Innovación</td><td>{b4_dl}/5</td><td>{'Alto' if b4_dl>3.5 else 'Medio' if b4_dl>2.5 else 'Bajo'}</td></tr>
<tr><td>Desempeño de Innovación</td><td>{b5_dl}/5</td><td>{'Alto' if b5_dl>3.5 else 'Medio' if b5_dl>2.5 else 'Bajo'}</td></tr>
</table>

<h2>3. Posición Estratégica</h2>
<div class="cuadrante">
<strong>Cuadrante:</strong> {'Líderes Estratégicos — Alta innovación y alto desempeño.' if (sum([b1_dl,b2_dl,b3_dl,b4_dl,b5_dl])/5)>2.75 and ssg_dl>50 else 'Innovadores no Rentables — Alta innovación, bajo desempeño económico.' if (sum([b1_dl,b2_dl,b3_dl,b4_dl,b5_dl])/5)>2.75 else 'Competitivos no Innovadores — Buen desempeño, baja innovación.' if ssg_dl>50 else 'Rezagados — Por debajo de la media en ambas dimensiones.'}
</div>

<h2>4. Notas y conclusiones del equipo directivo</h2>
<div class="nota">Espacio para anotaciones, reflexiones y próximos pasos</div>

<hr/><p style='color:#9ca3af;font-size:.78rem;text-align:center;'>Plataforma Etelvia · Motor de Inteligencia Competitiva 360° · {hoy_dl}</p>
</body></html>"""
...
def generar_word_benchmarking():
    try:
        ...
        import io
        doc = Document()
        t = doc.add_heading('Informe de Benchmarking Estratégico', 0)
        t.alignment = WD_ALIGN_PARAGRAPH.CENTER
        doc.add_paragraph(f"Empresa: {sector_dl} · {tam_dl} · {reg_dl} · {hoy_dl}")
        doc.add_heading('1. Posición Competitiva Global', level=1)
        tabla = doc.add_table(rows=1, cols=3)
        tabla.style = 'Table Grid'
        hdr = tabla.rows[0].cells
        hdr[0].text = 'Índice'; hdr[1].text = 'Valor'; hdr[2].text = 'Nivel'
        for nombre, val in [('SSG · Score Global', ssg_dl),('ICE · Competitividad', ice_dl),
            ...
            ('IDC · Dinamismo', idc_dl),('IIE · Exportación', iie_dl),('IPT · Productividad', ipt_dl)]:
            row = tabla.add_row().cells
            row[0].text = nombre
            row[1].text = f"{val}/100"
            row[2].text = 'Alto' if val>70 else 'Medio' if val>40 else 'Bajo'
        doc.add_heading('2. Perfil de Innovación', level=1)
        tabla2 = doc.add_table(rows=1, cols=3)
        tabla2.style = 'Table Grid'
        hdr2 = tabla2.rows[0].cells
        hdr2[0].text = 'Indicador'; hdr2[1].text = 'Puntuación'; hdr2[2].text = 'Nivel'
        for nombre, val in [('I+D+i', b1_dl),('Gestión Proyectos', b2_dl),
            ('Desarrollo Productos', b3_dl),('Estrategia Innovación', b4_dl),('Desempeño', b5_dl)]:
            row = tabla2.add_row().cells
            row[0].text = nombre
            row[1].text = f"{val}/5"
            row[2].text = 'Alto' if val>3.5 else 'Medio' if val>2.5 else 'Bajo'
